...
このアプリは、個人のローカルメモ帳として小さく保つことを前提にしています。拡張する場合は、まず日常の入力・検索・整理が楽になる機能から足すと扱いやすくなります。

- **編集機能**: 既存メモのタイトル・本文・タグ・状態を後から修正できるようにする。`updated_at` は編集時に更新します。
- **検索と絞り込み**: タイトル・本文・タグを対象にしたキーワード検索、状態別フィルター、タグ別フィルターを追加する。
- **並び替え**: 新しい順だけでなく、更新日時順、状態順、タグ順、重要メモ優先などを選べるようにする。
- **バックアップと書き出し**: `notes.db` のバックアップ、Markdown / CSV / JSON へのエクスポート、JSON からのインポートを用意する。
- **タグ管理**: よく使うタグの候補表示、タグ表記の揺れを直すリネーム機能、タグ一覧画面を追加する。
- **入力支援**: テンプレート、今日の日付の挿入、キーボードショートカット、保存後に本文へフォーカスを戻す操作を追加する。
- **表示の調整**: ダークモード、文字サイズ設定、状態ごとの表示/非表示、長い本文の折りたたみを追加する。
- **データ層の整理**: 機能追加で列を増やす場合は、`db.py` に移行処理を追加し、既存の `notes.db` を壊さずに更新できるようにする。

外部公開、複数端末同期、認証、クラウド保存を追加する場合は、このアプリの性格が変わります。実装前に、保存先、認証方式、バックアップ、紛失時の復旧、既存 `notes.db` からの移行方法を README に明記してください。
